...
// DIRECTIVE-COMPLIANT LAYOUT CONSTANTS
const Y_SPINE = 680; // Main process spine - slightly above vertical center
const Y_UPPER = Y_SPINE - 180; // Upper zone for input control
const Y_LOWER = Y_SPINE + 220; // Lower zone for output control
...
const SIZES = {
  F1: { w: 90, h: 140 },
  E1: { w: 280, h: 160 },
  R1: { w: 180, h: 200 }, // 2-bed reactor
  E2: { w: 200, h: 180 },
  D1: { w: 300, h: 140 },
};

// Valve Positions - STRICTLY ZONED
const VALVES = {
  TCV01B: { x: 510, y: Y_SPINE }, // On spine
  TCV01A: { x: 740, y: Y_UPPER }, // Upper zone
  TCV02A: { x: 1380, y: Y_UPPER }, // Upper zone
  TCV02B: { x: 1180, y: Y_LOWER }, // Lower zone
  TCV03A: { x: 1680, y: Y_LOWER }, // Lower zone
};
...
export default function ProcessMap({
  ...
  // 2-bed system
  const twoBedsystem = bedImbalance ? [bedImbalance.beds[0], bedImbalance.beds[1]].filter(Boolean) : [];
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-6 relative">
      <svg viewBox="0 0 2560 1200" className="w-full h-auto">
        ...
        {/* ========== SPINE ZONE: NORMAL PROCESS FLOW ========== */}
        ...
        <line x1={ANCHORS.F1.x + SIZES.F1.w/2} y1={Y_SPINE} x2={VALVES.TCV01B.x - 20} y2={Y_SPINE} stroke="#555" strokeWidth="4" />
        ...
        <g transform={`translate(${VALVES.TCV01B.x}, ${VALVES.TCV01B.y})`} onClick={() => handleUnitClick('tcv01b')} className={cn(interactive && "cursor-pointer")}>
          ...
          {interactive && <text x="0" y="-20" fill="#aaa" fontSize="16" textAnchor="middle" fontWeight="500">TCV-01B</text>}
        </g>
        
        {/* SPINE: TCV-01B to E-1 */}
        <line x1={VALVES.TCV01B.x + 20} y1={Y_SPINE} x2={ANCHORS.E1.x - SIZES.E1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="4" />
        ...
        {/* MAIN EXCHANGER E-1 */}
        <g transform={`translate(${ANCHORS.E1.x}, ${ANCHORS.E1.y})`} onClick={() => handleUnitClick('e1')} className={cn(interactive && "cursor-pointer")}>
          ...
          {[-50, -35, -20, -5, 10, 25, 40, 55].map(y => (
            <line key={y} x1={-SIZES.E1.w/2 + 15} y1={y} x2={SIZES.E1.w/2 - 15} y2={y} stroke="#444" strokeWidth="1.5" opacity="0.9" />
          ))}
          <text x="0" y={SIZES.E1.h/2 + 24} fill="#aaa" fontSize="22" textAnchor="middle" fontWeight="600">E-1</text>
        </g>

        {/* SPINE: E-1 to R-1 */}
        <line x1={ANCHORS.E1.x + SIZES.E1.w/2} y1={Y_SPINE} x2={ANCHORS.R1.x - SIZES.R1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="4" />
        ...
        {/* REACTOR R-1 (2-BED) */}
        <g transform={`translate(${ANCHORS.R1.x}, ${ANCHORS.R1.y})`} onClick={() => handleUnitClick('r1')} className={cn(interactive && "cursor-pointer")}>
            ...
            return (
              <g key={bed.id}>
                ...
                {Array.from({ length: 84 }).map((_, pidx) => {
                  ...
                  return (
                    <circle key={pidx} cx={px} cy={py} r="2" fill={bedColor} opacity={0.6 + glowIntensity} className="transition-all duration-500" />
                  );
                })}
                {interactive && (
                  <text x={-SIZES.R1.w/2 - 24} y={yStart + bedHeight / 2 + 4} fill="#666" fontSize="16" textAnchor="middle">B{bed.id}</text>
                )}
              </g>
            );
          })}
          ...
          {escalationLevel >= 2 && (
            <rect x={-SIZES.R1.w/2 + 5} y={-SIZES.R1.h/2 + 5} width={SIZES.R1.w - 10} height={SIZES.R1.h - 10} fill="url(#reactorGlow)" opacity="1.08" className={escalationLevel >= 3 ? "animate-[pulse_1.2s_ease-in-out_infinite]" : ""} />
          )}
          
          <text x="0" y={SIZES.R1.h/2 + 24} fill="#aaa" fontSize="22" textAnchor="middle" fontWeight="bold">R-1</text>
        </g>

        {/* REACTOR OUTLET - DROPS VERTICALLY FIRST (DIRECTIVE RULE) */}
        <line x1={ANCHORS.R1.x} y1={ANCHORS.R1.y + SIZES.R1.h/2} x2={ANCHORS.R1.x} y2={ANCHORS.R1.y + SIZES.R1.h/2 + 60} stroke="#555" strokeWidth="4" />
        <circle cx={ANCHORS.R1.x} cy={ANCHORS.R1.y + SIZES.R1.h/2 + 30} r="4" fill={shellThermalColor}>
          <animate attributeName="cy" values={`${ANCHORS.R1.y + SIZES.R1.h/2};${ANCHORS.R1.y + SIZES.R1.h/2 + 60}`} dur={animationSpeed} repeatCount="indefinite" />
        </circle>

        {/* REACTOR READOUT */}
        {interactive && (
          <>
            <text x={ANCHORS.R1.x + SIZES.R1.w/2 + 20} y={ANCHORS.R1.y - SIZES.R1.h/2 - 14} fill="#888" fontSize="18" textAnchor="start">Outlet</text>
            <text x={ANCHORS.R1.x + SIZES.R1.w/2 + 20} y={ANCHORS.R1.y - SIZES.R1.h/2 + 6} fill={baseColor} fontSize="20" textAnchor="start" fontWeight="700" className="transition-colors duration-400">
              {currentTemp.toFixed(1)}{units}
            </text>
            {slope > 0 && (
              <text x={ANCHORS.R1.x + SIZES.R1.w/2 + 20} y={ANCHORS.R1.y - SIZES.R1.h/2 + 24} fill={baseColor} fontSize="16" textAnchor="start" className="transition-colors duration-400">
        ...
        <g opacity={valveStates.tcv01a === "OOS" ? 0.3 : 1}>
          <line x1={VALVES.TCV01B.x + 20} y1={Y_SPINE} x2={VALVES.TCV01B.x + 20} y2={Y_UPPER} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={VALVES.TCV01B.x + 20} y1={Y_UPPER} x2={ANCHORS.E1.x + SIZES.E1.w/2} y2={Y_UPPER} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={ANCHORS.E1.x + SIZES.E1.w/2} y1={Y_UPPER} x2={ANCHORS.E1.x + SIZES.E1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          ...
          <g transform={`translate(${VALVES.TCV01A.x}, ${VALVES.TCV01A.y})`} onClick={() => handleUnitClick('tcv01a')} className={cn(interactive && "cursor-pointer")}>
            ...
            {interactive && <text x="0" y="-20" fill="#aaa" fontSize="16" textAnchor="middle" fontWeight="500">TCV-01A</text>}
          </g>
        ...
        {/* SHELL INLET CONTROL TCV-02A */}
        <g>
          <line x1={ANCHORS.R1.x} y1={ANCHORS.R1.y + SIZES.R1.h/2 + 60} x2={VALVES.TCV02A.x} y2={ANCHORS.R1.y + SIZES.R1.h/2 + 60} stroke="#555" strokeWidth="3" />
          <line x1={VALVES.TCV02A.x} y1={ANCHORS.R1.y + SIZES.R1.h/2 + 60} x2={VALVES.TCV02A.x} y2={Y_UPPER + 14} stroke="#555" strokeWidth="3" />
          
          <g transform={`translate(${VALVES.TCV02A.x}, ${Y_UPPER})`} onClick={() => handleUnitClick('tcv02a')} className={cn(interactive && "cursor-pointer")}>
            <polygon points="-9,-9 9,-9 7,0 9,9 -9,9 -7,0" fill={valveStates.tcv02a === "OPEN" ? "#2F5D80" : "#B47A1F"} stroke="#555" strokeWidth="2" />
            {interactive && <text x="0" y="-20" fill="#aaa" fontSize="16" textAnchor="middle" fontWeight="500">TCV-02A</text>}
          </g>
          
          <line x1={VALVES.TCV02A.x} y1={Y_UPPER - 14} x2={VALVES.TCV02A.x} y2={Y_SPINE} stroke="#555" strokeWidth="3" />
          <line x1={VALVES.TCV02A.x} y1={Y_SPINE} x2={ANCHORS.E1.x + SIZES.E1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="3" />
          
          {valveStates.tcv02a !== "CLOSED" && (
            <>
              <circle cx={(ANCHORS.R1.x + VALVES.TCV02A.x)/2} cy={ANCHORS.R1.y + SIZES.R1.h/2 + 60} r="4" fill={shellThermalColor}>
                <animate attributeName="cx" values={`${ANCHORS.R1.x};${VALVES.TCV02A.x}`} dur={animationSpeed} repeatCount="indefinite" />
              </circle>
              <circle cx={(VALVES.TCV02A.x + ANCHORS.E1.x)/2} cy={Y_SPINE} r="4" fill={shellThermalColor}>
        ...
        <g opacity={valveStates.tcv02b === "CLOSED" ? 0.3 : 1}>
          <line x1={ANCHORS.R1.x} y1={ANCHORS.R1.y + SIZES.R1.h/2 + 60} x2={VALVES.TCV02B.x} y2={ANCHORS.R1.y + SIZES.R1.h/2 + 60} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={VALVES.TCV02B.x} y1={ANCHORS.R1.y + SIZES.R1.h/2 + 60} x2={VALVES.TCV02B.x} y2={Y_LOWER - 14} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          
          <g transform={`translate(${VALVES.TCV02B.x}, ${Y_LOWER})`} onClick={() => handleUnitClick('tcv02b')} className={cn(interactive && "cursor-pointer")}>
            <polygon points="-9,-9 9,-9 7,0 9,9 -9,9 -7,0" fill={valveStates.tcv02b === "OPEN" ? "#B47A1F" : "#333"} stroke="#555" strokeWidth="2" />
            {interactive && <text x="0" y="32" fill="#aaa" fontSize="16" textAnchor="middle" fontWeight="500">TCV-02B</text>}
          </g>
          
          <line x1={VALVES.TCV02B.x} y1={Y_LOWER + 14} x2={VALVES.TCV02B.x} y2={Y_SPINE} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={VALVES.TCV02B.x} y1={Y_SPINE} x2={ANCHORS.E1.x - SIZES.E1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          
          {valveStates.tcv02b !== "CLOSED" && (
            <circle cx={(VALVES.TCV02B.x + ANCHORS.E1.x)/2} cy={Y_SPINE} r="4" fill="#B47A1F">
              <animate attributeName="cx" values={`${VALVES.TCV02B.x};${ANCHORS.E1.x - SIZES.E1.w/2}`} dur={animationSpeed} repeatCount="indefinite" />
            </circle>
          )}
        </g>

        {/* SPINE: E-1 Shell Out to E-2 */}
        <line x1={ANCHORS.E1.x - SIZES.E1.w/2} y1={Y_SPINE} x2={ANCHORS.E2.x - SIZES.E2.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="4" />
        ...
        {/* EFFLUENT COOLER E-2 */}
        <g transform={`translate(${ANCHORS.E2.x}, ${ANCHORS.E2.y})`} onClick={() => handleUnitClick('e2')} className={cn(interactive && "cursor-pointer", coolingCapacity === "CONSTRAINED" && "animate-[wiggle_2s_ease-in-out_infinite]")}>
          ...
          <text x="0" y={SIZES.E2.h/2 + 24} fill="#aaa" fontSize="22" textAnchor="middle" fontWeight="600">E-2</text>
        </g>

        {/* COOLER BYPASS TCV-03A */}
        <g opacity={valveStates.tcv03a === "CLOSED" ? 0.3 : 1}>
          <line x1={ANCHORS.E2.x - SIZES.E2.w/2} y1={Y_SPINE} x2={ANCHORS.E2.x - SIZES.E2.w/2} y2={Y_LOWER} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={ANCHORS.E2.x - SIZES.E2.w/2} y1={Y_LOWER} x2={ANCHORS.E2.x + SIZES.E2.w/2} y2={Y_LOWER} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          <line x1={ANCHORS.E2.x + SIZES.E2.w/2} y1={Y_LOWER} x2={ANCHORS.E2.x + SIZES.E2.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="3" strokeDasharray="6,6" />
          ...
          <g transform={`translate(${VALVES.TCV03A.x}, ${VALVES.TCV03A.y})`} onClick={() => handleUnitClick('tcv03a')} className={cn(interactive && "cursor-pointer")}>
            ...
            {interactive && <text x="0" y="32" fill="#aaa" fontSize="16" textAnchor="middle" fontWeight="500">TCV-03A</text>}
          </g>
        ...
        {/* SPINE: E-2 to D-1 */}
        <line x1={ANCHORS.E2.x + SIZES.E2.w/2} y1={Y_SPINE} x2={ANCHORS.D1.x - SIZES.D1.w/2} y2={Y_SPINE} stroke="#555" strokeWidth="4" />
        ...
        <g transform={`translate(${ANCHORS.D1.x}, ${ANCHORS.D1.y})`} onClick={() => handleUnitClick('d1')} className={cn(interactive && "cursor-pointer")}>
          ...
          <text x="0" y={-SIZES.D1.h/2 + 30} fill="#888" fontSize="14" textAnchor="middle">Gas</text>
          
          <rect x={-SIZES.D1.w/2 + 10} y={-SIZES.D1.h/2 + 38} width={SIZES.D1.w - 20} height={28} fill="#B47A1F" opacity="0.2" />
          <text x="0" y={-SIZES.D1.h/2 + 58} fill="#D4A547" fontSize="14" textAnchor="middle">Naphtha</text>
          
          <rect x={-SIZES.D1.w/2 + 10} y={-SIZES.D1.h/2 + 66} width={SIZES.D1.w - 20} height={20} fill="#2F5D80" opacity="0.2" />
          <text x="0" y={-SIZES.D1.h/2 + 80} fill="#2F5D80" fontSize="14" textAnchor="middle">Water</text>
          
          {/* Gas outlet rises vertically */}
          <line x1="0" y1={-SIZES.D1.h/2} x2="0" y2={-SIZES.D1.h/2 - 40} stroke="#888" strokeWidth="2.5" />
          {interactive && <text x="0" y={-SIZES.D1.h/2 - 48} fill="#888" fontSize="14" textAnchor="middle">Gas Out</text>}
          
          <text x="0" y={SIZES.D1.h/2 + 24} fill="#aaa" fontSize="22" textAnchor="middle" fontWeight="600">D-1</text>
        </g>

        {/* ========== SUPPORT SYSTEM: H2 (SUBORDINATE, VERTICAL ONLY) ========== */}
        {equipment.h2Compressor && (
          <g opacity="0.4">
            <line x1={ANCHORS.R1.x - SIZES.R1.w/2 - 40} y1={ANCHORS.R1.y - SIZES.R1.h/2 - 80} x2={ANCHORS.R1.x - SIZES.R1.w/2 - 40} y2={ANCHORS.R1.y - SIZES.R1.h/2} stroke="#4A90E2" strokeWidth="2" strokeDasharray="4,4" />
            {interactive && (
              <text x={ANCHORS.R1.x - SIZES.R1.w/2 - 40} y={ANCHORS.R1.y - SIZES.R1.h/2 - 88} fill="#4A90E2" fontSize="14" textAnchor="middle">H₂ System</text>
            )}
            <circle cx={ANCHORS.R1.x - SIZES.R1.w/2 - 40} cy={ANCHORS.R1.y - SIZES.R1.h/2 - 40} r="3" fill="#4A90E2" opacity="0.6">
        ...
        {interactive && (
          <>
            <g transform="translate(200, 200)">
              <rect x="0" y="0" width="200" height="36" rx="6" fill="#1e1e1e" stroke="#444" strokeWidth="1.5" />
              <text x="100" y="23" fill={sensorQuality === "good" ? "#0F9F9F" : sensorQuality === "suspect" ? "#D4A547" : "#D4653F"} fontSize="16" textAnchor="middle" fontWeight="500">
                Sensor: {sensorQuality.toUpperCase()}
              </text>
            </g>
            <g transform="translate(420, 200)">
              <rect x="0" y="0" width="160" height="36" rx="6" fill="#1e1e1e" stroke="#444" strokeWidth="1.5" />
              <text x="80" y="23" fill="#999" fontSize="16" textAnchor="middle" fontWeight="500">
                {opMode === "steady" ? "Steady" : "Transient"}
              </text>
            </g>
          </>
        )}
      ...
      {interactive && selectedUnit && (
        <div className="absolute top-4 right-4 bg-[#1e1e1e] border-2 border-[#444] rounded-lg p-4 max-w-[280px] shadow-xl">
          ...
          {selectedUnit === 'r1' && (
            <>
              <h4 className="text-[#aaa] text-sm font-bold mb-2">R-1 Reactor (2-Bed)</h4>
