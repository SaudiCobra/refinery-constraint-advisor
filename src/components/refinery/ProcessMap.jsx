  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-6 relative">
      <svg viewBox="0 0 1200 540" className="w-full h-auto">
        ...
        <g>
          {/* Step 1: Drop from R-1 outlet DOWN 40px (clears reactor bottom) */}
          ...
          {/* Step 2: Horizontal LEFT under reactor to align with E-1 */}
          <line x1="560" y1="320" x2="290" y2="320" stroke="#555" strokeWidth="3" />
          {valveStates.tcv02a !== "CLOSED" && (
            <circle cx="425" cy="320" r="3" fill={shellThermalColor}>
              <animate attributeName="cx" values="560;290" dur={animationSpeed} repeatCount="indefinite" />
            </circle>
          )}
          
          {/* Step 3: Vertical UP to E-1 shell inlet nozzle */}
          <line x1="290" y1="320" x2="290" y2="292" stroke="#555" strokeWidth="3" />
          {valveStates.tcv02a !== "CLOSED" && (
            <circle cx="290" cy="306" r="3" fill={shellThermalColor}>
              <animate attributeName="cy" values="320;292" dur={animationSpeed} repeatCount="indefinite" />
            </circle>
          )}
          
          {/* TCV-02A positioned on the horizontal run under reactor */}
          <g transform="translate(425, 320)" onClick={() => handleUnitClick('tcv02a')} className={cn(interactive && "cursor-pointer")}>
            <polygon points="-10,-8 10,-8 8,0 10,8 -10,8 -8,0" fill={valveStates.tcv02a === "OPEN" ? "#2F5D80" : "#B47A1F"} stroke="#555" strokeWidth="2" />
            <text x="0" y="26" fill="#aaa" fontSize="9" textAnchor="middle" fontWeight="600">TCV-02A</text>
          </g>
        </g>

        {/* LOWER BRANCH: TCV-02B Shell Bypass (BELOW reactor) - MOVED DOWN 25px */}
        <g opacity={valveStates.tcv02b === "CLOSED" ? 0.3 : 1}>
          ...
          {/* TCV-02B positioned BELOW and to the right of reactor - moved down */}
