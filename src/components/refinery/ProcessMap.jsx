    ...
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-6 relative" style={presentationStyle}>
      <svg viewBox="0 0 2560 1200" className="w-full h-auto">
        ...
        <g transform={`translate(${ANCHORS.E1.x}, ${ANCHORS.E1.y})`} onClick={() => handleUnitClick('e1')} className={cn(interactive && "cursor-pointer hover:opacity-90 transition-all duration-400")}>
          <ellipse cx={-SIZES.E1.w/2} cy="0" rx="10" ry={SIZES.E1.h/2 - 8} fill="#1a1a1a" stroke={interactive || (preheatActive && preheatStatus?.includes("stress")) ? preheatColor : "#555"} strokeWidth={interactive || (preheatActive && preheatStatus?.includes("stress")) ? "2.5" : "2"} />
          <rect x={-SIZES.E1.w/2} y={-SIZES.E1.h/2 + 8} width={SIZES.E1.w} height={SIZES.E1.h - 16} fill="#2a2a2a" stroke={interactive || (preheatActive && preheatStatus?.includes("stress")) ? preheatColor : "#555"} strokeWidth={interactive || (preheatActive && preheatStatus?.includes("stress")) ? "3" : "2"} filter="url(#equipmentShadow)" />
          <ellipse cx={SIZES.E1.w/2} cy="0" rx="10" ry={SIZES.E1.h/2 - 8} fill="#2a2a2a" stroke={interactive || (preheatActive && preheatStatus?.includes("stress")) ? preheatColor : "#555"} strokeWidth={interactive || (preheatActive && preheatStatus?.includes("stress")) ? "2.5" : "2"} />
                ...
                {Array.from({ length: 72 }).map((_, pidx) => {
                  ...
                  return (
                    <circle
                      ...
                      r="2"
                      fill={bedColor}
                      opacity={interactive ? (0.6 + glowIntensity) : (0.3 + glowIntensity * 0.5)}
                      className="transition-all duration-500"
                    />
                  );
                })}
        ...
        <g transform={`translate(${ANCHORS.E2.x}, ${ANCHORS.E2.y})`} onClick={() => handleUnitClick('e2')} className={cn(interactive && "cursor-pointer hover:opacity-90 transition-all duration-400", interactive && coolingCapacity === "CONSTRAINED" && "animate-[wiggle_2s_ease-in-out_infinite]")}>
          <rect x={-SIZES.E2.w/2} y={-SIZES.E2.h/2} width={SIZES.E2.w} height={SIZES.E2.h} rx="10" fill="#2a2a2a" stroke={interactive || coolingCapacity === "CONSTRAINED" ? coolerColor : "#555"} strokeWidth={interactive || coolingCapacity === "CONSTRAINED" ? "3" : "2"} filter="url(#equipmentShadow)" className="transition-all duration-400" />
